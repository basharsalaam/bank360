...
import { getSign, hasOnlyZeros, numberWithCommas } from "../../utils/helpers";
...
class BarAndLineChart extends React.Component {
                ...
                <div className="tool-tip-flex">
                  <div className="tool-tip-flex-left">
                    <div className="tool-tip-flex-top">
                      {w.globals.initialSeries[1].data.length === 0 ? (
                        <></>
                      ) : (
                        <Icons.ArrowDownIcon />
                      )}
                    ...
                    <p
                      ...
                      style={{
                        color:
                          w.globals.initialSeries[2].data[dataPointIndex] >= 0
                            ? "#54B773"
                            : "#E3452F",
                        textTransform: "lowercase",
                      }}
                    >
                      {getSign(w.globals.initialSeries[2].data[dataPointIndex])}{" "}
                      {numberWithCommas(
                        Math.abs(
                          Math.round(
                            w.globals.initialSeries[2].data[dataPointIndex]
                          )
                        )
                      )}{" "}
                      % from {w.globals.initialSeries[4].data[0]}
                    </p>
                  </div>
                  {w.globals.initialSeries[1].data.length > 0 && (
                    <div className="tool-tip-flex-right">
                      ...
                      <p
                        ...
                        style={{
                          color:
                            w.globals.initialSeries[3].data[dataPointIndex] >= 0
                              ? "#54B773"
                              : "#E3452F",
                          textTransform: "lowercase",
                        }}
                      >
                        {getSign(
                          w.globals.initialSeries[3].data[dataPointIndex]
                        )}{" "}
                        {numberWithCommas(
                          Math.abs(
                            Math.round(
  ...
  render() {
    return (
      // check if every node is 0
      hasOnlyZeros(this.props.inflowData) &&
